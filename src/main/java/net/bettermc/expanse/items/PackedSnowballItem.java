                ...
                if (j <= 0 || playerEntity.isTouchingWaterOrRain()) {
                    if (!world.isClient) {
                        ...
                        if (j == 0) {
                            ...
                            world.spawnEntity(packedSnowballEntity);
                            world.playSoundFromEntity((PlayerEntity)null, packedSnowballEntity, SoundEvents.ITEM_TRIDENT_THROW, SoundCategory.PLAYERS, 1.0F, 1.0F);
                            if (!playerEntity.getAbilities().creativeMode) {
