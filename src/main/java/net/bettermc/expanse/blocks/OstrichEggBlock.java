...
public class OstrichEggBlock  extends  Block {
    ...
    public OstrichEggBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    private void tryBreakEgg(World world, BlockState state, BlockPos pos, Entity entity, int inverseChance) {
        if (this.breaksEgg(world, entity)) {
            if (!world.isClient && world.random.nextInt(inverseChance) == 0 && state.isOf(ModBlocks.OSTRICH_EGG)) {
        ...
        if (this.shouldHatchProgress(world) && isSandBelow(world, pos)) {
            ...
            if (i < 2) {
                ...
                for (int j = 0; j < (Integer) state.get(EGGS); ++j) {
                    world.syncWorldEvent(2001, pos, Block.getRawIdFromState(state));
                    OstrichEntity ostrichEntity = (OstrichEntity) ModEntities.OSTRICH.create(world);
                    ostrichEntity.setBreedingAge(-24000);
                    ostrichEntity.setHomePos(pos);
                    ostrichEntity.refreshPositionAndAngles((double) pos.getX() + 0.3 + (double) j * 0.2, (double) pos.getY(), (double) pos.getZ() + 0.3, 0.0F, 0.0F);
                    world.spawnEntity(ostrichEntity);
                }
            }
        }
    ...
    private boolean breaksEgg(World world, Entity entity) {
        if (!(entity instanceof OstrichEntity) && !(entity instanceof BatEntity)) {
